...
(
    ...
    FOREIGN KEY (post_id) REFERENCES board_free_table (b_no) ON DELETE CASCADE,
    FOREIGN KEY (c_writer) REFERENCES ACCOUNT_TABLE_HDH (user_NICKNAME) ON DELETE CASCADE
);

create sequence Free_posts_comment_seq;

insert into Free_posts_comment (b_no, c_writer, c_content, c_date)
values (223, '호잇이계속되면', '좋은글입니다.', systimestamp);
select *
from Free_posts_comment;
